...
async def get_blast_radius(analysis_id: str, node_id: str, depth: int = 3) -> dict[str, int]:
    ...
    async with driver.session(database="neo4j") as session:
        # Variable-length path: start -[*1..depth]-> node; count by label
        result = await session.run(
            """
            MATCH (start {id: $nodeId, analysisId: $analysisId})
            MATCH path = (start)-[*1..$depth]->(node {analysisId: $analysisId})
            WITH DISTINCT node, labels(node) AS lbls
            RETURN
              count(CASE WHEN 'File' IN lbls THEN 1 END) AS files,
              count(CASE WHEN 'Function' IN lbls THEN 1 END) AS functions,
              count(CASE WHEN 'Endpoint' IN lbls THEN 1 END) AS endpoints
            """,
            nodeId=node_id,
            analysisId=analysis_id,
            depth=depth,
